...
namespace WebAPITest.Controllers
{
    ...
    public class RGDialogsController : ControllerBase
    {
        ...
        private static readonly List<RGDialogsClients> DialogContext = Init();

        [HttpPost]
        public Guid Get(GetRGDialogsDto getRgDialogsDto)
        {
            var result = DialogContext
                .GroupBy(x => x.IDRGDialog)
                .FirstOrDefault(x =>
                    x.All(d => getRgDialogsDto.UsersIds.Contains(d.IDClient)))?.Key;
